...
const leaveRequestSchema = z.object({
  ...
  startDate: z.string({required_error: 'Start date is required.'}).refine(val => val, { message: 'Start date is required.' }),
  endDate: z.string({required_error: 'End date is required.'}).refine(val => val, { message: 'End date is required.' }),
...
export async function chatbotAction(
  ...
  try {
    ...
    const result = await runChatbotFlow(validatedFields.data as ChatbotInput);
